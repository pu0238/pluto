use crate::{method::Method, all_or_some::AllOrSome, http::HttpResponse};
use std::ops::Deref;
...
impl Cors {
    ...
    pub(crate) fn credentials(mut self, value: bool) -> Self {
    ...
    pub(crate) fn exposed_headers(mut self, headers: Vec<&str>) -> Self {
    ...
    /// passed headers and returns changed CORS
    pub(crate) fn allow_headers(mut self, headers: Vec<&str>) -> Self {
    ...
    pub(crate) fn allow_methods(mut self, methods: Vec<Method>) -> Self {
    ...
    pub(crate) fn merge(&self, response: &mut HttpResponse) {
